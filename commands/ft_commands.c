...
/*   Updated: 2024/10/18 02:49:13 by ccodere          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	ft_create_tokens -> 0 : Success / 1 : Error / 2 : Syntaxe
	external_cmds		-> 0 : Success / 1 : Error / 127 : CmdNotfound
	call_commands		-> if built-in commands return 127,
				call_commands is called
						by the child process. Return value depend of child
						return value.
*/
int	parse_input(t_minishell *ms, char *input)
{
	ms->ret = ft_create_tokens(ms, input);
	if (ms->ret == SUCCESS)
	{
...
/*
	Commands that must be call in the parent process to work Return 0 if the
	command is successful, 1 if not command couldn't execute, and -1 if we
	didnt find the command in this function. Also return -1 if the tokens is
	NULL for the moment.
*/
...
/*	to do:
	...
	from the ft_check_n_create_path and from the related command).

	- Each command should return an int to get the return value. We will need it
